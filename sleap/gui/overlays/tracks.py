...
class TrackTrailOverlay(BaseOverlay):
    ...
    def _get_trail_point(
        self, inst, node_index: Optional[int]
        ...
        point_index = point_indices.get(node_names[node_index])
        if point_index is None or not inst.points["visible"][point_index]:
            return None

        xy = inst.points["xy"][point_index]
